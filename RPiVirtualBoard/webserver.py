...
# Nombre o dirección IP del sistema anfitrión del
#servidor web
address = "localhost"
#address = "192.168.1.254"
# Puerto en el cual el servidor estará atendiendo
# solicitudes HTTP
...
class WebServer(BaseHTTPRequestHandler):
    ...
    def _serve_ui_file(self):
        if not os.path.isfile("user_interface.html"):
            ...
            return
        try:
            with open("user_interface.html", "r") as f: content = "\n".join(f.readlines())
        except:
    ...
    GET, es
     decir, páginas. Por seguridad, no se analizan
    variables que lleguen
     por esta vía"""

def do_GET(self):
# Revisamos si se accede a la raiz.
# En ese caso se responde con la interfaz por
# defecto
    if self.path == '/':
        self.send_response(200)
...
# Fin de cabecera
        self.end_headers()
 # Por simplicidad, se devuelve como respuesta el
#contenido del
 # archivo html con el código de la página de
#interfaz de usuario
        self._serve_ui_file()
 # En caso contrario, se verifica que el archivo
#exista y se sirve
    else:
...
"""do_POST controla todas las solicitudes recibidas vía POST, es decir, envíos de formulario. Aquí se gestionan los comandos para la Raspberry Pi"""
def do_POST(self):
...
# HTTPRequestHandler definido en este archivo
    ...
    try:
# Mantiene al servidor web ejecutándose en segundo
#plano
...
# Maneja la interrupción de cierre CTRL+C
        ...
        print(sys.exc_info())
 # Detiene el servidor web cerrando todas las
#conexiones
